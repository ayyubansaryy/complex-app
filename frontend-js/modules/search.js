...
export default class search{
    ...
    renderResultsHTML(posts) {
      if (posts.length) {
        ...
        <div class="list-group-item active"><strong>Search Results</strong> (${posts.length > 1 ? `${posts.length} item found` : `${posts.length} items found`})</div>
        ${posts.map(posts => {
          let = postDate = post.createDate 
        ...
        this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm"> Sorry, we could not find any results`
      }
      this.hideLoaderIcon()
